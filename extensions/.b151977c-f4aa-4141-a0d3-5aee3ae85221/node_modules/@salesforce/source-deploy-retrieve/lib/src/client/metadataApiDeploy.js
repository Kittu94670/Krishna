...
const envVars_1 = require("@salesforce/core/envVars");
...
class MetadataApiDeploy extends metadataTransfer_1.MetadataTransfer {
    ...
    constructor(options) {
        ...
        this.registry = options.registry ?? new registry_1.RegistryAccess();
        if (this.mdapiTempDir) {
            this.mdapiTempDir = (0, node_path_1.join)(this.mdapiTempDir, `${new Date().toISOString().replace(/[<>:"\\|?*]/g, '_')}_deploy`);
        }
    }
    /**
    ...
    async deployRecentValidation(rest = false) {
        if (!this.id) {
            throw new sfError_1.SfError(messages.getMessage('error_no_job_id', ['deploy']), 'MissingJobIdError');
        }
    ...
    async pre() {
        const LifecycleInstance = lifecycle_1.Lifecycle.getInstance();
        const connection = await this.getConnection();
    ...
    async post(result) {
        const lifecycle = lifecycle_1.Lifecycle.getInstance();
        const connection = await this.getConnection();
        try {
            const apiVersion = connection.getApiVersion();
    ...
    async warnIfDeployThresholdExceeded(zipSize, zipFileCount) {
        const thresholdPercentage = Math.abs(envVars_1.envVars.getNumber('SF_DEPLOY_SIZE_THRESHOLD', 80));
        ...
        if (zipSize > fileSizeThreshold) {
            await lifecycle_1.Lifecycle.getInstance().emitWarning(`Deployment zip file size is approaching the Metadata API limit (~39MB). Warning threshold is ${thresholdPercentage}% and size ${zipSize} > ${fileSizeThreshold}`);
        }
        if (zipFileCount && zipFileCount > fileCountThreshold) {
            await lifecycle_1.Lifecycle.getInstance().emitWarning(`Deployment zip file count is approaching the Metadata API limit (10,000). Warning threshold is ${thresholdPercentage}% and count ${zipFileCount} > ${fileCountThreshold}`);
        }
    }
    async getZipBuffer() {
        ...
        if (this.options.zipPath) {
            if (!graceful_fs_1.default.existsSync(this.options.zipPath)) {
                throw new sfError_1.SfError(messages.getMessage('error_path_not_found', [this.options.zipPath]));
            }
        ...
        if (this.options.components && this.components) {
            const converter = new convert_1.MetadataConverter(this.registry);
            const { zipBuffer, zipFileCount } = await converter.convert(this.components, 'metadata', { type: 'zip' });
            if (!zipBuffer) {
                throw new sfError_1.SfError(messages.getMessage('zipBufferError'));
            }
...
[...messageMap.keys()].flatMap((key) => {
    ...
    if (!fr.find((c) => c.type === type && c.fullName === fullName) &&
        ...
        void lifecycle_1.Lifecycle.getInstance().emitWarning(`${deployMessage.componentType ?? '<no component type in deploy message>'}, ${deployMessage.fullName}, returned from org, but not found in the local project`);
    }
});
...
const buildFileResponsesFromComponentSet = (cs) => (response) => {
    ...
    const fileResponses = (cs.getSourceComponents().toArray() ?? [])
        .flatMap((deployedComponent) => (0, deployMessages_1.createResponses)(cs.projectDirectory)(deployedComponent, responseMessages.get((0, deployMessages_1.toKey)(deployedComponent)) ?? []).concat(deployedComponent.type.children
        ? deployedComponent.getChildren().flatMap((child) => {
            const childMessages = responseMessages.get((0, deployMessages_1.toKey)(child));
            return childMessages ? (0, deployMessages_1.createResponses)(cs.projectDirectory)(child, childMessages) : [];
        })
        : []))
...
const validateOptions = (options) => {
    ...
    if (runningRelevantTestsOnly && beforeApiV66) {
        throw new sfError_1.SfError(messages.getMessage('error_invalid_test_level', ['RunRelevantTests', '66.0']), 'InvalidTestLevelSelection');
    }
};
