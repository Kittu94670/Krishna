...
class ComponentSetBuilder {
    ...
    static async build(options) {
        ...
        if (metadata && !org && !sourcepath?.length) {
            ...
            metadata.metadataEntries = replacedEntries.filter((entry) => typeof entry === 'string' && entry.length > 0);
        ...
        if (org) {
            componentSet ??= new componentSet_1.ComponentSet(undefined, registry);
            const { componentSet: orgComponentSet, botVersionFilters: orgBotVersionFilters } = await this.resolveOrgComponents(registry, options);
    ...
    static async resolveOrgComponents(registry, options) {
        ...
        if (metadata) {
            if (metadata.metadataEntries?.length) {
                ...
                metadata.metadataEntries = replacedEntries.filter((entry) => typeof entry === 'string' && entry.length > 0 && entry.includes(':'));
                if (orgBotVersionFilters.length > 0) {
        ...
        const componentSet = await componentSet_1.ComponentSet.fromConnection({
...
const getOrgComponentFilter = (org, mdMap, metadata, registry) => metadata?.metadataEntries?.length
    ? (component) => {
...
const replacePseudoTypes = async (pseudoTypeInfo) => {
    ...
    mdOption.metadataEntries.map((rawEntry) => {
        ...
        if (Object.values(PSEUDO_TYPES).includes(typeName)) {
            ...
            const { baseBotName, versionFilter } = (0, agentResolver_1.parseBotVersionFilter)(botName);
    ...
    if (pseudoEntries.length) {
        await Promise.all(pseudoEntries.map(async (pseudoEntry) => {
            ...
            if (pseudoType === PSEUDO_TYPES.AGENT) {
                ...
                const { baseBotName, versionFilter } = (0, agentResolver_1.parseBotVersionFilter)(pseudoName);
                botVersionFilters.push({ botName: baseBotName, versionFilter });
