...
exports.WebApplicationsSourceAdapter = void 0;
/*
...
class WebApplicationsSourceAdapter extends bundleSourceAdapter_1.BundleSourceAdapter {
    ...
    populate(trigger, component, isResolvingSource = true) {
        ...
        const expectedXmlPath = (0, node_path_1.join)(contentPath, `${appName}.webapplication-meta.xml`);
        ...
        if (isResolvingSource) {
            ...
            const contentEntries = (this.tree.readDirectory(contentPath) ?? []).filter((entry) => entry !== xmlFileName && entry !== 'webapplication.json');
            if (contentEntries.length === 0) {
                // For deploy/source, we expect at least one non-metadata content file (e.g. index.html).
                throw new sfError_1.SfError(messages.getMessage('error_expected_source_files', [contentPath, this.type.name]), 'ExpectedSourceFilesError');
            }
            if (!this.tree.exists(descriptorPath)) {
                throw new sfError_1.SfError(messages.getMessage('error_expected_source_files', [descriptorPath, this.type.name]), 'ExpectedSourceFilesError');
            }
...
exports.WebApplicationsSourceAdapter = WebApplicationsSourceAdapter;
//# sourceMappingURL=webApplicationsSourceAdapter.js.map