...
const global_1 = require("@salesforce/core/global");
...
class TreeContainer {
    ...
    find(fileType, name, directory) {
        const fileName = this.readDirectory(directory).find((entry) => {
            const parsed = (0, path_1.parseMetadataXml)(this.getUpdatedFsPath((0, node_path_1.join)(directory, entry)));
            const metaXmlCondition = fileType === 'metadataXml' ? !!parsed : !parsed;
            return (0, path_1.baseName)(entry) === name && metaXmlCondition;
        });
        if (fileName) {
            return this.getUpdatedFsPath((0, node_path_1.join)(directory, fileName));
        }
    }
    /** if the container has cwd set, apply it to the path */
    getUpdatedFsPath(fsPath) {
        return this.cwd && !fsPath.startsWith(this.cwd) ? (0, node_path_1.join)(this.cwd, fsPath) : fsPath;
    }
}
...
class NodeFSTreeContainer extends TreeContainer {
    isDirectory(fsPath) {
        ...
        return (0, graceful_fs_1.statSync)(this.getUpdatedFsPath(fsPath)).isDirectory();
    }
    exists(fsPath) {
        return (0, graceful_fs_1.existsSync)(this.getUpdatedFsPath(fsPath));
    }
    readDirectory(fsPath) {
        return (0, graceful_fs_1.readdirSync)(this.getUpdatedFsPath(fsPath));
    }
    readFile(fsPath) {
        // significant enough performance increase using sync instead of fs.promise version
        return Promise.resolve((0, graceful_fs_1.readFileSync)(this.getUpdatedFsPath(fsPath)));
    }
    readFileSync(fsPath) {
        return (0, graceful_fs_1.readFileSync)(this.getUpdatedFsPath(fsPath));
    }
    stream(fsPath) {
        if (!this.exists(fsPath)) {
            throw new Error(`File not found: ${fsPath}`);
        }
        return (0, graceful_fs_1.createReadStream)(this.getUpdatedFsPath(fsPath));
    }
}
...
class ZipTreeContainer extends TreeContainer {
    ...
    isDirectory(fsPath) {
        ...
        throw new sfError_1.SfError(messages.getMessage('error_path_not_found', [fsPath]), 'LibraryError');
    }
    readDirectory(fsPath) {
        const resolvedPath = this.match(fsPath);
        ...
        throw new sfError_1.SfError(messages.getMessage('error_expected_directory_path', [fsPath]), 'LibraryError');
    }
    async readFile(fsPath) {
        ...
        if (resolvedPath) {
            ...
            throw new sfError_1.SfError(`Expected a file at path ${fsPath} but found a directory.`);
        }
        throw new sfError_1.SfError(messages.getMessage('error_expected_file_path', [fsPath]), 'LibraryError');
    }
    ...
    stream(fsPath) {
        ...
        if (resolvedPath) {
            ...
            if (jsZipObj && !jsZipObj.dir) {
                ...
                return new node_stream_1.Readable(global_1.Global.isWeb ? { highWaterMark: 1 } : {}).wrap(jsZipObj.nodeStream());
            }
            throw new sfError_1.SfError(messages.getMessage('error_no_directory_stream', [this.constructor.name]), 'LibraryError');
        }
        throw new sfError_1.SfError(messages.getMessage('error_expected_file_path', [fsPath]), 'LibraryError');
    }
    ...
    match(fsPath) {
        ...
        return this.zipKeyMap.get(node_path_1.posix.normalize(fsPath.replaceAll('\\', '/')));
    }
    ensureDirectory(dirPath) {
        ...
        throw new sfError_1.SfError(messages.getMessage('error_path_not_found', [dirPath]), 'LibraryError');
    }
}
    ...
    populate(virtualFs) {
        for (const dir of virtualFs) {
            ...
            for (const child of children) {
                ...
                if (!dirPathFromTree) {
                    throw new sfError_1.SfError(`The directory at path ${dirPath} does not exist in the virtual file system.`);
                }
