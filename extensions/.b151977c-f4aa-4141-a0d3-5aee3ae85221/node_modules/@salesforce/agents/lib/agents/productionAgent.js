...
class ProductionAgent extends agentBase_1.AgentBase {
    ...
    async getBotMetadata() {
        if (!this.botMetadata) {
            ...
            this.botMetadata = await this.connection.singleRecordQuery(`SELECT ${botDefinitionFields}, (SELECT ${botVersionFields} FROM BotVersions WHERE IsDeleted = false ORDER BY VersionNumber) FROM BotDefinition WHERE ${whereClause} LIMIT 1`);
